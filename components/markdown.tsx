...
const NonMemoizedMarkdown = ({ children }: { children: string }) => {
  const components: Partial<Components> = {
    ...
    a: ({ children, ...props }) => {
      return (
        // @ts-expect-error pre
        <Link
          className="ml-1 rounded-md bg-primary-100 px-2 py-1 text-white hover:bg-primary-50 hover:transition-all hover:animate-in"
          target="_blank"
          rel="noreferrer"
          {...props}
        >
  ...
  return (
    <ReactMarkdown
      className={`max-w-full rounded-xl border border-zinc-700 bg-dark-400 p-3 text-white sm:max-w-[85%]`}
